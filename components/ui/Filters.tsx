        ...
        <div className='flex flex-wrap gap-2'>
            {data.map((filter)=>(
                <div className="flex items-center" key={filter?.id}>
                    <Button onClick={()=>handleClick(filter?.id)} className={cn("rounded-md text-sm text-gray-800 pb-2 bg-white border border-gray-300", selectedValue&&"bg-black text-white")}>{filter?.name}</Button>
                </div>
            ))}
        </div>
    </div>
